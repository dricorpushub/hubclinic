...
const state = {
  ...
  bonusPeriod: localStorage.getItem('hub_bonus_period') || '2026-07',
  bonusEntries: JSON.parse(localStorage.getItem('hub_bonus_entries') || 'null') || HUB_DATA.team.map((p,i)=>({...p,id:'bonus-'+i,period:'2026-07',status:'Em apuração'}))
};
const nav = [
  ['','dashboard','⌂','Dashboard'],
...
function bonusEntries(period=state.bonusPeriod){return state.bonusEntries.filter(x=>x.period===period)}
function periodName(p){const [y,m]=p.split('-');return new Date(+y,+m-1,1).toLocaleDateString('pt-BR',{month:'long',year:'numeric'}).replace(/^./,x=>x.toUpperCase())}
function managerBonus(entries){
  const tiers=HUB_DATA.bonusRules.gerencia.faixas;
  return entries.filter(x=>!['Priscila','Driele'].includes(x.name)).reduce((sum,p)=>{
    const a=stageBonus(HUB_DATA.bonusRules.vendas53,p.vendas53),b=stageBonus(HUB_DATA.bonusRules.vendas54,p.vendas54);
    const ia=HUB_DATA.bonusRules.vendas53.faixas.findIndex(f=>f.meta===a.meta),ib=HUB_DATA.bonusRules.vendas54.faixas.findIndex(f=>f.meta===b.meta);
    const idx=Math.max(ia,ib); return sum+(idx>=0?(tiers[Math.min(idx,tiers.length-1)]?.bonus||0):0)
  },0)
}
function bonusTotals(entries){const people=entries.map(personResult),individual=people.filter(x=>x.name!=='Driele').reduce((s,x)=>s+x.bonus,0),management=managerBonus(entries);return {people,individual,management,total:individual+management,sales:entries.reduce((s,x)=>s+x.vendas53+x.vendas54+x.produtos,0)}}
function bonificacao(){
  const entries=bonusEntries(),tot=bonusTotals(entries),periods=[...new Set(state.bonusEntries.map(x=>x.period))].sort().reverse();
  return head('Bonificação','Lance os resultados do mês e deixe o sistema calcular tudo automaticamente.',`<button class="btn btn-secondary" onclick="exportBonusCSV()">Exportar CSV</button><button class="btn btn-primary" onclick="bonusForm()">+ Novo lançamento</button>`)+
  `<div class="card bonus-toolbar"><div><span class="eyebrow">PERÍODO</span><select id="bonusPeriod">${periods.map(p=>`<option value="${p}" ${p===state.bonusPeriod?'selected':''}>${periodName(p)}</option>`).join('')}</select></div><div class="bonus-toolbar-actions"><button class="btn btn-secondary" onclick="newBonusMonth()">Abrir novo mês</button><button class="btn btn-primary" onclick="closeBonusMonth()">Fechar mês</button></div></div>
  <div class="kpi-grid">${kpi('Bônus individual',BRL.format(tot.individual),entries.length+' lançamentos','◇')}${kpi('Bônus de gerência',BRL.format(tot.management),'Separado do ranking','☆')}${kpi('Total previsto',BRL.format(tot.total),'Individual + gerência','◎')}${kpi('Vendas consideradas',BRL.format(tot.sales),'Base da apuração','↗')}</div>
  <div class="tabs bonus-tabs"><button class="tab active" data-btab="lancamentos">Lançamentos</button><button class="tab" data-btab="ranking">Ranking</button><button class="tab" data-btab="regras">Regras</button><button class="tab" data-btab="historico">Histórico</button><button class="tab" onclick="showSimulator()">Simulador</button></div>
  <div id="bonusContent">${bonusLaunchView(entries)}</div>`
}
function bonusLaunchView(entries){return `<div class="card panel"><div class="panel-head"><div><h3>Apuração do mês</h3><span>Cálculo automático pelas regras oficiais</span></div><span class="tag">${entries.length} pessoas</span></div><div class="table-wrap"><table class="data-table"><thead><tr><th>Colaborador</th><th>Consultas</th><th>Avaliações</th><th>Vendas 5.3</th><th>Vendas 5.4</th><th>Produtos</th><th>Bônus</th><th>Status</th><th></th></tr></thead><tbody>${entries.map(p=>{const r=personResult(p);return `<tr><td><strong>${p.name}</strong><br><span style="color:var(--muted);font-size:11px">${p.role}</span></td><td>${p.consultas}</td><td>${p.avaliacoes}</td><td>${BRL.format(p.vendas53)}</td><td>${BRL.format(p.vendas54)}</td><td>${BRL.format(p.produtos)}</td><td class="money">${BRL.format(r.bonus)}</td><td><span class="tag ${p.status==='Fechado'?'green':'amber'}">${p.status}</span></td><td><button class="mini-btn" onclick="bonusForm('${p.id}')">Editar</button></td></tr>`}).join('')||`<tr><td colspan="9"><div class="empty"><strong>Nenhum lançamento</strong>Adicione o primeiro lançamento do mês.</div></td></tr>`}</tbody></table></div></div>`}
function bonusRankingView(){const e=bonusEntries(),t=bonusTotals(e),r=t.people.filter(x=>x.name!=='Driele').sort((a,b)=>b.bonus-a.bonus),pri=t.people.find(x=>x.name==='Priscila');return `<div class="grid-2"><div class="card panel"><div class="panel-head"><h3>Ranking individual</h3><span>Gerência não entra</span></div>${r.map((p,i)=>`<div class="rank-row"><div class="rank-pos">${i+1}</div><div class="rank-name"><strong>${p.name}</strong><span>${p.role}</span></div><div class="rank-value"><strong>${BRL.format(p.bonus)}</strong></div></div>`).join('')}</div><div class="card panel"><div class="panel-head"><h3>Painel da gerência</h3><span>Priscila</span></div><div class="management-total"><span>Bônus próprio</span><strong>${BRL.format(pri?.bonus||0)}</strong></div><div class="management-total"><span>Bônus gerência</span><strong>${BRL.format(t.management)}</strong></div><div class="management-total grand"><span>Total Priscila</span><strong>${BRL.format((pri?.bonus||0)+t.management)}</strong></div><div class="extra-rule">Gerência considera somente as vendas 5.3 e 5.4 da equipe.</div></div></div>`}
function bonusRulesView(){const r=HUB_DATA.bonusRules;return `<div class="bonus-grid">${['consultas','avaliacoes','vendas53','vendas54'].map(k=>{const x=r[k];return `<div class="card bonus-card"><div class="bonus-title"><div><h3>${x.nome}</h3><p>${x.unidade}</p></div></div><div class="tiers">${x.faixas.map(f=>`<div class="tier ${f.nome.includes('Preta')?'black':''}"><span>${f.nome}</span><strong>${x.unidade==='R$'?BRL.format(f.meta):f.meta}</strong><span>${BRL.format(f.bonus)}</span></div>`).join('')}</div>${x.excedente?`<div class="extra-rule">Após 40: + ${BRL.format(x.excedente.valor_unitario)} por excedente.</div>`:''}</div>`}).join('')}</div><div class="card panel" style="margin-top:16px"><span class="tag purple">Produtos: 5%</span> <span class="tag red">Até faixa 3: vermelho</span> <span class="tag amber">Faixas 4 e 5: amarelo</span> <span class="tag green">Faixa 6 em diante: verde</span></div>`}
function bonusHistoryView(){const periods=[...new Set(state.bonusEntries.map(x=>x.period))].sort().reverse();return `<div class="card panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>Período</th><th>Pessoas</th><th>Vendas</th><th>Individual</th><th>Gerência</th><th>Total</th><th>Status</th></tr></thead><tbody>${periods.map(p=>{const e=bonusEntries(p),t=bonusTotals(e);return `<tr><td><strong>${periodName(p)}</strong></td><td>${e.length}</td><td>${BRL.format(t.sales)}</td><td>${BRL.format(t.individual)}</td><td>${BRL.format(t.management)}</td><td class="money">${BRL.format(t.total)}</td><td><span class="tag ${e.every(x=>x.status==='Fechado')?'green':'amber'}">${e.every(x=>x.status==='Fechado')?'Fechado':'Em apuração'}</span></td></tr>`}).join('')}</tbody></table></div></div>`}
function setBonusTab(tab){$$('.bonus-tabs .tab').forEach(b=>b.classList.toggle('active',b.dataset.btab===tab));$('#bonusContent').innerHTML=tab==='ranking'?bonusRankingView():tab==='regras'?bonusRulesView():tab==='historico'?bonusHistoryView():bonusLaunchView(bonusEntries())}
function bonusForm(id){const old=state.bonusEntries.find(x=>x.id===id),names=[...new Set(HUB_DATA.team.map(x=>x.name))],p=old||{id:'bonus-'+Date.now(),period:state.bonusPeriod,name:names[0],role:'Comercial',consultas:0,avaliacoes:0,vendas53:0,vendas54:0,produtos:0,status:'Em apuração'};modal(`<h2>${old?'Editar':'Novo'} lançamento</h2><div class="form-grid"><div class="field"><label>Período</label><input id="bp" type="month" value="${p.period}"></div><div class="field"><label>Colaborador</label><select id="bn">${names.map(n=>`<option ${n===p.name?'selected':''}>${n}</option>`).join('')}</select></div><div class="field"><label>Função</label><input id="br" value="${p.role}"></div><div class="field"><label>Status</label><select id="bs"><option ${p.status==='Em apuração'?'selected':''}>Em apuração</option><option ${p.status==='Fechado'?'selected':''}>Fechado</option></select></div><div class="field"><label>Consultas pagas</label><input id="bc" type="number" min="0" value="${p.consultas}"></div><div class="field"><label>Avaliações/reconsultas</label><input id="ba" type="number" min="0" value="${p.avaliacoes}"></div><div class="field"><label>Vendas 5.3</label><input id="b53" type="number" step=".01" min="0" value="${p.vendas53}"></div><div class="field"><label>Vendas 5.4</label><input id="b54" type="number" step=".01" min="0" value="${p.vendas54}"></div><div class="field full"><label>Produtos</label><input id="bpr" type="number" step=".01" min="0" value="${p.produtos}"></div></div><div class="modal-actions">${old?`<button class="btn btn-danger" onclick="deleteBonus('${p.id}')">Excluir</button>`:''}<button class="btn btn-secondary" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="saveBonusEntry('${p.id}')">Salvar</button></div>`)}
function saveBonusEntry(id){const x={id,period:$('#bp').value,name:$('#bn').value,role:$('#br').value,consultas:+$('#bc').value,avaliacoes:+$('#ba').value,vendas53:+$('#b53').value,vendas54:+$('#b54').value,produtos:+$('#bpr').value,status:$('#bs').value};if(state.bonusEntries.some(e=>e.id!==id&&e.period===x.period&&e.name===x.name)){toast('Já existe lançamento desta pessoa neste mês');return}const i=state.bonusEntries.findIndex(e=>e.id===id);if(i>=0)state.bonusEntries[i]=x;else state.bonusEntries.push(x);state.bonusPeriod=x.period;localStorage.setItem('hub_bonus_period',x.period);save('hub_bonus_entries',state.bonusEntries);closeModal();render();toast('Lançamento salvo')}
function deleteBonus(id){if(confirm('Excluir este lançamento?')){state.bonusEntries=state.bonusEntries.filter(x=>x.id!==id);save('hub_bonus_entries',state.bonusEntries);closeModal();render()}}
function newBonusMonth(){modal(`<h2>Abrir novo mês</h2><div class="field"><label>Período</label><input id="newBonusPeriod" type="month" value="2026-08"></div><div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancelar</button><button class="btn btn-primary" onclick="createBonusMonth()">Criar lançamentos</button></div>`)}
function createBonusMonth(){const p=$('#newBonusPeriod').value;if(!p)return;if(!state.bonusEntries.some(x=>x.period===p)){HUB_DATA.team.forEach((x,i)=>state.bonusEntries.push({...x,id:'bonus-'+Date.now()+i,period:p,consultas:0,avaliacoes:0,vendas53:0,vendas54:0,produtos:0,status:'Em apuração'}));save('hub_bonus_entries',state.bonusEntries)}state.bonusPeriod=p;localStorage.setItem('hub_bonus_period',p);closeModal();render();toast('Novo mês aberto')}
function closeBonusMonth(){const e=bonusEntries();if(!e.length){toast('Não há lançamentos');return}if(confirm('Fechar a apuração deste mês?')){e.forEach(x=>x.status='Fechado');save('hub_bonus_entries',state.bonusEntries);render();toast('Mês fechado')}}
function exportBonusCSV(){const rows=bonusEntries().map(p=>{const r=personResult(p);return [p.period,p.name,p.role,p.consultas,p.avaliacoes,p.vendas53,p.vendas54,p.produtos,r.bonus,p.status]});const csv=[['Período','Colaborador','Função','Consultas','Avaliações','Vendas 5.3','Vendas 5.4','Produtos','Bônus','Status'],...rows].map(r=>r.join(';')).join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv'}));a.download='bonificacao-'+state.bonusPeriod+'.csv';a.click()}
...
function render(){
 ...
 if(state.page==='bonificacao'){ $('#bonusPeriod').onchange=e=>{state.bonusPeriod=e.target.value;localStorage.setItem('hub_bonus_period',e.target.value);render()}; $$('.bonus-tabs .tab[data-btab]').forEach(b=>b.onclick=()=>setBonusTab(b.dataset.btab)); }
 window.scrollTo(0,0);
}
render();
